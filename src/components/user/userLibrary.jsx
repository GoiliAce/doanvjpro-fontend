...
export const SongFavorite = () => {
    const [songFavorite, setSongFavorite] = useState([]);
    ...
    useEffect(() => {
        fetchData('user/add-song-user','song', 20).then((data) => {
            setSongFavorite(data.songs);
            console.log(data);
        });
    }, []);
    return (
        <div className="container song-favorite">
            <RecentlyTitle>
                <box-icon name='heart' color='#ff0a0a'  type='solid'></box-icon>
                <h3 className="m-0">Favorite Songs</h3>
            </RecentlyTitle>
            <RecenlyPlayedWrapper>
                {
                    songFavorite.map((song, index) => (
                        <RecentlyItem key={index}>
                            <img src={song.thumbnail} alt="" />
                            <RecentlyItemTitle >{song.title}</RecentlyItemTitle>
                            <RecentlyItemMore >
                                <box-icon name='heart' color='#ff0000' type='solid'></box-icon>
                                <box-icon name='play' color='#ffffff' onClick={()=>handleSongClick(index,songFavorite )}></box-icon>
