...
export const ChangePassword = ({ account }) => {
    ...
    const handleModelSubmit = async (event) => {
        ...
        // check upper case and lower case and number
        const regex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/;
        if (!regex.test(data['n-pass'])) {
            
            setNotRegex(true);
            return;
        }
        if (data['n-pass'] !== data['r-pass']) {
            alert('Mật khẩu nhập lại không khớp');
            setNotmatch(true);
            return;
        }
        account.password = data['n-pass'];
        const res = await checkPassword(data['c-pass']);
        ...
        if (res) {
            ...
            if (result) {
                ...
                dispatch(setAccountLogin(account));
                setButtonType('');
                setWrongpass(false);
                setNotmatch(false);
                setNotRegex(false);
    ...
    return (
        <div className="user-setting__form_update">
            ...
            <div className="user-setting__form_update__title_description">
                Để đổi mật khẩu, vui lòng nhập mật khẩu cũ và mật khẩu mới
            </div>
            <div className="user-setting__form_update__content">
                <form className='frmChangePass' onSubmit={handleModelSubmit}>
                    <div class="form-group">
                        <label for="c-pass">Nhập mật khẩu cũ</label>
                        <input type="password" class="form-control" id="c-pass" name='c-pass' required />
                        {wrongpass && <small className="text-danger">Mật khẩu cũ không đúng</small>}
                    </div>
                    <div class="form-group">
                        <label for="n-pass">Nhập mật khẩu mới</label>
                        <input type="password" class="form-control" id="n-pass" name='n-pass' required />
                        {notRegex && <small className="text-danger">Mật khẩu phải có ít nhất 8 ký tự, bao gồm chữ hoa, chữ thường và số</small>}
                    </div>
                    <div class="form-group">
                        <label for="r-pass">Nhập lại mật khẩu mới</label>
                        <input type="password" class="form-control" id="r-pass" name='r-pass' required />
                        {notMatch && <small className="text-danger">Mật khẩu nhập lại không khớp</small>}
                    </div>
